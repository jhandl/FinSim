...
Focuses on the raw `EconomicData.convert()` API. It asserts directional behavior (strong→weak currencies multiply, weak→strong divide), 1% round-trip idempotency, same-currency identity, sign/zero preservation, finite outputs (<1e15), and expected divergence between evolution/constant/PPP/reversion modes. It is intentionally strict on arithmetic invariants but flexible on absolute values—tolerances are relative so the test survives recalibration of FX series. Any new conversion mode or change in inflation lookup must still pass these invariants, with **evolution FX treated as the canonical default.**
