...
class IndexFunds extends InvestmentAsset {
  constructor(growth, stdev = 0) {
    // Resolve ruleset and type definition
    var ruleset = null;
    try {
      var cfg = Config.getInstance();
    ...
    var typeDef = null;
    if (ruleset && typeof ruleset.findInvestmentTypeByKey === 'function') {
      typeDef = ruleset.findInvestmentTypeByKey('indexFunds');
    }
    
    // Construct default typeDef if missing (legacy IE defaults)
    if (!typeDef) {
        typeDef = {
            key: 'indexFunds',
            label: 'Index Funds',
            ...
            taxation: {
                exitTax: {
                    ...
                    eligibleForAnnualExemption: true, // legacy IE behavior
    ...
    super(typeDef, growth, stdev, ruleset);
    
    // Force legacy hardcoded overrides if typeDef was incomplete
    if (!this.baseCurrency) this.baseCurrency = 'EUR';
    if (!this.assetCountry) this.assetCountry = 'ie';
  }
}

class Shares extends InvestmentAsset {
  constructor(growth, stdev = 0) {
    var ruleset = null;
    try {
      var cfg = Config.getInstance();
    ...
    var typeDef = null;
    if (ruleset && typeof ruleset.findInvestmentTypeByKey === 'function') {
      typeDef = ruleset.findInvestmentTypeByKey('shares');
    }

    if (!typeDef) {
        var cgtRate = (ruleset && typeof ruleset.getCapitalGainsRate === 'function') ? ruleset.getCapitalGainsRate() : 0.33;
        typeDef = {
            key: 'shares',
            label: 'Shares',
            baseCurrency: 'EUR',
