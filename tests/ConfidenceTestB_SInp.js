...
module.exports = {
  name: 'C_B-SInp',
  description: 'Verifies non-pension salary (SInp) income and associated taxes.',
  category: 'confidence',
  isCustomTest: true,
  async runCustomTest() {
    const params = microParams({ targetAge: 31 });
    const events = [
      { type: 'SInp', id: 'salary', amount: 10000, fromAge: 30, toAge: 30, currency: 'AAA', rate: 0, match: 0 }
    ];
    const scenarioDef = {
      name: 'C_B-SInp',
      description: 'Verifies non-pension salary (SInp) income and associated taxes.',
    ...
    if (!row30) {
      ...
      // Pension fund should remain 0
      if (Math.abs(row30.pensionFund - 0) > 1) errors.push(`Expected pensionFund ≈ 0, got ${row30.pensionFund}`);
    }
