class Equity {
  ...
  class ETF extends Equity {
    ...
    constructor(growth, stdev=0) {
      super(revenue.etfExitTax, growth, stdev);
    }
    ...
    addYear() {
      ...
      for (let i = 0; i < this.portfolio.length; i++) {
        if ((revenue.etfDeemedDisposalYears > 0) && (this.portfolio[i].age % revenue.etfDeemedDisposalYears === 0)) {
  ...
  class InvestmentTrust extends Equity {
    ...
    constructor(growth, stdev=0) {
      super(revenue.cgtRate, growth, stdev);
    }
  ...
  class Pension extends Equity {
    ...
    getLumpsum() {
      this.lumpSum = true;
      let amount = this.sell(this.capital() * revenue.pensionLumpSumLimit);
      this.lumpSum = false;
      return amount;
    }
    
    drawdown() {
      let minimumDrawdown = revenue.pensionMinDrawdownAgeLimits.reduce(
          (acc, limit) => (age > limit ? revenue.pensionMinDrawdownLimits[limit] : acc), 
          revenue.pensionMinDrawdownLimits[revenue.pensionMinDrawdownAgeLimits[0]]
      );
